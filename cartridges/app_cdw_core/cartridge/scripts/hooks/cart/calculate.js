...
function calculateProductPrices (basket) {
    ...
    if(customer.registered && customer.profile && customer.profile.custom && customer.profile.custom.b2bUser && customer.profile.custom.b2bAccountNumber) {
        ...
        {
            b2bPriceMap = HookMgr.callHook('app.s2k.b2b.price', 'GetB2BPriceForProducts', basket,b2bAccountNumber);
    ...
    while (productLineItems.hasNext()) {
        ...
        if(productLineItem.custom && "fromStoreId" in productLineItem.custom) {
            fromStoreId =  productLineItem.custom.fromStoreId;
        }
        else if(!empty(fromStoreId))
        {
            productLineItem.custom.fromStoreId = fromStoreId;
        }
        ...
        // we have to update the product price as well as the bonus adjustment
            ...
            var b2bPricekey = productLineItem.productID+'~'+ productLineItem.quantity;
            if(b2bPriceMap!=null && b2bPriceMap.containsKey(b2bPricekey))
            {
                price = b2bPriceMap.get(b2bPricekey);
            }
        ...
        if(b2bPriceMap!=null && b2bPriceMap.containsKey(b2bPricekey))
        {
            var price = b2bPriceMap.get(b2bPricekey);
            productLineItem.setPriceValue(price.valueOrNull);
            productLineItem.custom.b2bPriceQuantity = new Number(productLineItem.quantity);
        }
    }
}
