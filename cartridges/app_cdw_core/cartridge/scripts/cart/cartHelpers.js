...
function addProductToCart(currentBasket, productId, quantity, childProducts, options, storeId, req) {
    ...
    if (product.bundle) {
        ...
        if('maxOrderQuantity' in product.custom)
        {
            maxOrdQty = product.custom.maxOrderQuantity
        }
        maxOrdQtyExceeded = totalQtyRequested>maxOrdQty;
    ...
    if (productInCart) {
        ...
        /** CUSTOM LOCIC TO DISABLE THE MULTI SHIPPING AND ENABLE ERROR HANDLING FOR BOPIS --- BEGIN */
        if (inStorePickupEnabled) {
            ...
            // Check if the existing cart has the BOPIS
            var existingStoreIdInCart;
            var alreadyItemsExists = false;
            collections.forEach(productLineItems, function (item) {
                alreadyItemsExists = true;
                if(item.custom && "fromStoreId" in item.custom) {
                    existingStoreIdInCart =  item.custom.fromStoreId;
                }
            });

            if(alreadyItemsExists) {
                if(existingStoreIdInCart != null && existingStoreIdInCart != undefined) { // This means, BOPIS item already exists in the cart
                    session.custom.bopisStore = existingStoreIdInCart;
                    if(storeId != null) { //This means, incoming request is also for BOPIS
                        if(storeId != existingStoreIdInCart) { //This means, the incoming add2cart is for different store, so throw error
                            var storeObject = StoreMgr.getStore(existingStoreIdInCart);
                            var storeName = storeObject.name.toString().split(",");
                            result.error = true;
                            result.message = Resource.msgf('error.alert.selected.shipment.store.cannot.be.added', 'product', null, storeName);
                        } else { // This means, incoming Add2Cart is also for BOPIS and for the same store
                            shipment = defaultShipment;
                        }
                    } else { // This means, incoming Add2Cart is for regular Shipment, so throw error
                        result.error = true;
                        result.message = Resource.msg('error.alert.selected.shipment.regular.cannot.be.added', 'product', null);
                    }
                } else { // This means, the existing item in the cart is for regular Shipment
                    if(storeId != null ) { //This means, incoming Add2Cart is for BOPIS, so throw error
                        result.error = true;
                        result.message = Resource.msg('error.alert.selected.shipment.store.incoming.cannot.be.added', 'product', null);
                    } else {
                            shipment = defaultShipment;
                    }
                ...
                // This is the first item getting added, it doesnt matter BOPIS or Shipped options
                if(storeId != null) {//This means, creating the BOPIS shipment
                    session.custom.bopisStore = storeId;
                    var inStoreShipment = createInStorePickupShipmentForLineItem(currentBasket, storeId, req);
                    shipment = inStoreShipment || defaultShipment;
                } else{ //This means, creating the regular shipment
                    // shipment = defaultShipment;
                    shipment =  currentBasket.createShipment(UUIDUtils.createUUID());
        ...
        /** CUSTOM LOCIC TO DISABLE THE MULTI SHIPPING AND ENABLE ERROR HANDLING FOR BOPIS --- END */
