...
function updateWishlistPrivacyCache(customer, req, config) {
    ...
    var index=0;
    var itemsIterator = list.items.iterator();
    var listOfIdsStr = '';
    var Site = require('dw/system/Site');
    var wishListMaxProducts = Site.current.getCustomPreferenceValue('wishListMaxProducts') || 70; 
    while(itemsIterator.hasNext()){
        index = index +1;
        if(index<wishListMaxProducts){
            if(listOfIdsStr === ''){
                listOfIdsStr = itemsIterator.next().productID;
            }
            else{
                listOfIdsStr = listOfIdsStr +',' +itemsIterator.next().productID;
            }
        }
        else
            break;
    };
    req.session.privacyCache.set('wishlist', listOfIdsStr);
}
