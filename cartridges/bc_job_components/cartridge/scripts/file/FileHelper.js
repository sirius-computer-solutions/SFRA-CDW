...
const CATALOGS_PATH = File.SEPARATOR + File.CATALOGS + File.SEPARATOR;
const ZIP_FILE_REG_EXP = /\.zip$/;
var Logger = require('dw/system/Logger').getLogger('cs.job.FileHelper');
/**
 ...
 * @returns {String} The final generated path
 */
 var getCatalogsPath = function (filePath) {
    var path = CATALOGS_PATH;

    if (!empty(filePath)) {
        path = CATALOGS_PATH + filePath;
    }
 ...
 */
var unzipFiles = function (files, targetDirectoryPath, useArchiveNameAsFolder, removeArchivesAfterCompletion, sitesPath) {
    if (files.length === 0) {
        return false;
    }
    ...
    var targetDirectory;
    if(sitesPath) {
        targetDirectoryPath = getCatalogsPath(targetDirectoryPath) + (targetDirectoryPath.charAt(targetDirectoryPath.length - 1).equals(File.SEPARATOR) ? '' : File.SEPARATOR);
    } else {
    ...
    Logger.info("targetDirectoryPath::::"+targetDirectoryPath);
    ...
    return files.filter(function (file) {
        ...
        var archiveName = getFileName(file);
        Logger.info("Every archiveName::::"+archiveName);
        targetDirectory = createDirectory(targetDirectoryPath);
        Logger.info("Every targetDirectoryPath::::"+targetDirectoryPath);
