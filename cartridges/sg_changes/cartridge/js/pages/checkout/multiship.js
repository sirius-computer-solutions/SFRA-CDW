...
function addEditAddress(target) {
    ...
    if ($addressForm.find('select[name$="_country"]').val() != 'us') {
   	$addressForm.find('select[name$="_state"]').parents('.form-row').hide();
    }
   
    $addressForm.on('change', function(e) {
        $addressForm.data('ischanged' , true);
        $addressForm.data('isskip', false);
    });
   
    $(document).on('change','.multisuggestion [name=dwfrm_multishipping_editAddress]', function(e) {
   	  var selected = $(e.target).children(':selected').first();
        var selectedAddress = $(selected).data('address');
        if (!selectedAddress) { return; }
        util.fillAddressFields(selectedAddress, $addressForm);
        // re-validate the form
        $addressForm.validate().form();
        $addressForm.data('ischanged' , false);
    });
   
    $addressForm.on('change','select[name$="_country"]', function(e){
   	if (e.target.value != 'us') {
   		$addressForm.find('select[name$="_state"]').parents('.form-row').hide();
        } else {
       	$addressForm.find('select[name$="_state"]').parents('.form-row').show();
        }
    });
    ...
    $addressForm.attr('data-isskip', false);
    $addressForm.on('submit', function (e) {
        e.preventDefault();
        if (!$addressForm.valid()) {
            return false;
        }
        if(!$addressForm.data('isskip') && ($addressForm.find('select[name$="_country"]').val() == 'us')) {
            if ($addressForm.data('ischanged')) {
    	       $.ajax({
    	            type: 'POST',
    	            url: window.Urls.getSuggest +'?multishipping=true',
    	            data: $addressForm.serialize()
    	        }).done(function (response) {
    	        	if (!response.text) {
    	        		$addressForm.data('ischanged', !response.success);
    	        		$addressForm.data('isskip', response.success);
    	        		$addressForm.submit();
    	        	} else {
    	        		$addressForm.data('ischanged', false);
    	        		$addressForm.data('isskip', true);
    		        	$('.multisuggestion').remove();
    		        	$('#edit-address-form').before(response.text);
    	        	}
    	        });	      
        	   return false;
            }
