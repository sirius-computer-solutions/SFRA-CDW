...
function editAddresses() {
    ...
    if (cart) {
    	...
    	session.privacy.VertexAddressSelected = false;
    ...
    if (!app.getForm(session.forms.multishipping.editAddress.addressFields).copyTo(newAddress) || !app.getForm(session.forms.multishipping.editAddress.addressFields.states).copyTo(newAddress)) {
        ...
        if (addToCustomerAddressBook && addToCustomerAddressBook === true) {
            if (referenceAddress) {
                ...
                Transaction.wrap(function () {
                	customerAddress.custom.taxnumber = session.forms.multishipping.editAddress.addressFields.taxnumber.value;
                });
            }
            ...
            if (customerAddress) {
                ...
                helperAddress.custom =  { taxnumber:newAddress.taxnumber };
                Transaction.wrap(function () {
