...
function handleForm() {
    ...
    addressForm.handleAction({
        ...
        create: function () {
        	...
        	var createAddress = Address.create(session.forms.profile.address);
            if (!session.forms.profile.address.valid || !createAddress) {
                response.redirect(URLUtils.https('Address-Add'));
                success = false;
            }
        	
        	Transaction.wrap(function () {
        		createAddress.object.custom.taxnumber = request.httpParameterMap.dwfrm_profile_address_taxnumber.value;
        	});
        ...
        edit: function () {
            ...
            try {
                ...
                var taxnumber =  session.forms.profile.address.taxnumber.value;
                Transaction.wrap(function() {
                	updatedAddress.object.custom.taxnumber = taxnumber;
                });
                success = true;
            } catch (e) {
                success = false;
...
function edit() {
    ...
    app.getForm(profileForm.address).setValue('taxnumber', address.custom.taxnumber);
