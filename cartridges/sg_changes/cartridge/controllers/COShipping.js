...
function handleShippingSettings(cart) {
    Transaction.wrap(function () {
        ...
        shippingAddress.custom.taxnumber = session.forms.singleshipping.shippingAddress.addressFields.taxnumber.value;
...
function singleShipping() {
    ...
    singleShippingForm.handleAction({
        save: function () {
            ...
            var country = singleShippingForm.get('shippingAddress.addressFields').getValue('country');

	        var result = Vertex.LookupTaxAreas(singleShippingForm, cart.object);
	        if (!result) {
                start();
	            return;
            }
            ...
            if (customer.authenticated && session.forms.singleshipping.shippingAddress.addToAddressBook.value) {
                var newAddress = app.getModel('Profile').get(customer.profile).addAddressToAddressBook(cart.getDefaultShipment().getShippingAddress());
                var taxnumber = singleShippingForm.get('shippingAddress.addressFields').getValue('taxnumber');
                Transaction.wrap(function () {
                	newAddress.custom.taxnumber = taxnumber;
                });
            }
        ...
        // use direct require instead of app.getController, because app.js requires controllers only from the same cartridge (using ~/ paths)
        shipToMultiple: require('~/cartridge/controllers/COShippingMultiple').Start,
        error: function () {
