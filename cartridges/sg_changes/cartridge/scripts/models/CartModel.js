...
var CartModel = AbstractModel.extend({
    ...
    consolidateInStoreShipments: function () {
        ...
        var storeObject; var shippingAddress; var
            orderAddress;
    ...
    preCalculateShipping: function (shippingMethod) {
        ...
        if (shipment) {
            ...
            var productShippingDiscounts = new ArrayList(); // list holds all products shipping discounts NOT promotions e.g. fixed shipping discount or free shipping for individual products discount
            var productIter = this.getAllProductLineItems().iterator();
            var priceAdj;
            var priceAdjArray = []; // array to hold shipping price adjustments data (we have to create objects since price adjustments get lost after applying a shipping method
            var priceAdjIter = shipment.getShippingPriceAdjustments().iterator();
            var priceAdjTotal = new Money(0.0, currencyCode); // total of all price adjustments
            var surchargeTotal = new Money(0.0, currencyCode); // total of all surcharges
            var adustedSurchargeTotal = new Money(0.0, currencyCode); // total of all surcharges minus price adjustments
    ...
    addAddressToAddressBook: function (addressToAdd) {
        ...
        if (addressToAdd) {
            ...
            // Add customer tax number to new created address
            if ('taxnumber' in addressToAdd.custom) {
            	if (addressToAdd.custom.taxnumber != 'undefined') {
            		address.taxnumber = addressToAdd.custom.taxnumber;
            	}
            }
