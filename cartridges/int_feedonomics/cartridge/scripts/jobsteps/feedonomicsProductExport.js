...
function writeProductExportField(product, csvProductArray, columnValue) {
    switch (columnValue) {
        ...
        case FConstants.HEADER_VALUES.PRIMARY_CATEGORY:
            csvProductArray.push(FeedonomicsHelpers.getPrimaryCategory(product));
            break;
        // Product Categories
        ...
        case FConstants.HEADER_VALUES.IN_STOCK:
            ...
            break;
        // Product's Pre order/Backorder Allocation
        case FConstants.HEADER_VALUES.PREORDER_BACKORDER_ALLOCATION:
            csvProductArray.push(FeedonomicsHelpers.getPreorderBackorderAllocation(product));
            break;
        ...
        case FConstants.HEADER_VALUES.AVAILABILITY_STATUS:
            ...
            break;
        // Product's In Stock Date
        case FConstants.HEADER_VALUES.INSTOCK_DATE:
            csvProductArray.push(FeedonomicsHelpers.getInStockDate(product));
            break;
...
exports.process = function (product) { // eslint-disable-line consistent-return
    try {
        if ((!onlineOnly || product.isOnline()) && (!skipMaster || !product.isMaster())
            && (!availableOnly || FeedonomicsHelpers.isOrderable(product))) {
            var csvProductArray = [];
