...
function writeInventoryExportField(product, csvProductArray, columnValue) {
    switch (columnValue) {
        ...
        case FConstants.HEADER_VALUES.IN_STOCK:
            ...
            break;
        // Product's Pre order/ Back Order Allocation
...
exports.process = function (product) { // eslint-disable-line consistent-return
    try {
        if ((!onlineOnly || product.isOnline()) && (!skipMaster || !product.isMaster()) && (!availableOnly || FeedonomicsHelpers.isOrderable(product))) {
            var csvProductArray = [];
