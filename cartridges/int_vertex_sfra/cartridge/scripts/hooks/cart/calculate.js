...
function calculateProductPrices (basket) {
    ...
    if(customer.registered && 'profile' in customer && 'custom' in customer.profile && 'b2bAccountNumber' in customer.profile.custom)
    {
    ...
    while (productQuantitiesIt.hasNext()) {
        ...
        var key = prod.ID+'~'+ quantity;
        if(b2bPriceMap!=null && b2bPriceMap.containsKey(key))
        {
            productPrices.put(prod, b2bPriceMap.get(key));    
        }
        else{
