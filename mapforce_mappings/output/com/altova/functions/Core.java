...
public class Core {
	...
	public static IEnumerable groupBy(IEnumerable x, SequenceFactory y) throws Exception {
		HashMap<Object, SequenceCache> groups_cache = new HashMap<Object, SequenceCache>();
		SequenceCache result = new SequenceCache(null);;
		IEnumerator item_enumerator = x.enumerator();
		while (item_enumerator.moveNext()) {
			IEnumerator key_enumerator = distinctValues((IEnumerable)y.invoke(item_enumerator.current())).enumerator();
			while (key_enumerator.moveNext()) {
				SequenceCache group = groups_cache.get(key_enumerator.current());
				if (group == null) {
					group = new SequenceCache(null);
					groups_cache.put(key_enumerator.current(), group);
					result.append(new Group(key_enumerator.current(), group));
				}
	...
	public static BigDecimal round(BigDecimal n) { return n.setScale(0, n.signum() == -1 ? RoundingMode.HALF_DOWN : RoundingMode.HALF_UP); }
