...
public abstract class CalendarBase {
	...
	public void parseTime(String newvalue, int optional) {
		...
		try {
			...
			if (hour == 24)	{// if hour is 24 add a day.
				hour = 0;
	...
	public boolean parseDateTime(String s, int part) {
		...
		if (bDatePart) {
			...
			if ((part & (DateTimePart_Month | DateTimePart_Day)) != 0) {
				...
				if (( part & DateTimePart_Month ) != 0) {
					if (!context.readDigitAndAdvance(month, 10, 1 ))
						return false;
					if (!context.readDigitAndAdvance(month, 1, month.value < 10 ? 9 : 2 ))
						return false;
					if (month.value == 0)
						return false;
				}
				...
				if (( part & DateTimePart_Day ) != 0) {
					...
					if (!context.readDigitAndAdvance( day, 10, maxFirstDigit ))
						return false;
					if (!context.readDigitAndAdvance( day, 1, 9 ))
						return false;
		...
		if (bTimePart) {
			...
			// hour from 0 to 2
			if (!context.readDigitAndAdvance( hour, 10, 2 ))
				return false;
			if (!context.readDigitAndAdvance( hour, 1, hour.value < 20 ? 9 : 4 ))
				return false;
			if (!context.checkAndAdvance( ':' ))
				return false;
			int maxFirstDigit = hour.value == 24 ? 0 : 5;
			int maxSecondDigit = hour.value == 24 ? 0 : 9;
			if (!context.readDigitAndAdvance( minute, 10, maxFirstDigit ))
				return false;
			if (!context.readDigitAndAdvance( minute, 1, maxSecondDigit ))
				return false;
			if (!context.checkAndAdvance( ':' ))
				return false;
			RefInt secondInt = new RefInt (0);
			if (!context.readDigitAndAdvance( secondInt, 10, maxFirstDigit ))
				return false;
			if (!context.readDigitAndAdvance( secondInt, 1, maxSecondDigit ))
				return false;
		...
		if (context.checkAndAdvance('Z')) {
			...
			RefInt temp = new RefInt(0);
			if (!context.readDigitAndAdvance( temp, 600, 9 ))
				return false;
			if (!context.readDigitAndAdvance( temp, 60, 9 ))
				return false;
			if (!context.checkAndAdvance( ':' ))
				return false;
			if (!context.readDigitAndAdvance( temp, 10, 5 ))
				return false;
			if (!context.readDigitAndAdvance( temp, 1, 9 ))
				return false;
