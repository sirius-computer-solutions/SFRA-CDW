...
public class DataElement extends StructureItem {
	...
	public boolean read (Parser.Context context) {
		...
		// edi validate
		String sError = context.getValidator().validate( context.getParent().getParticle().getName(), context.getParticle().getName(), s.toString());
		if (sError.length() > 0)
			context.handleError( 
	...
	public void write (Writer writer, ITextNode node, Particle particle) throws IOException {
		...
		//edi validate
		String sError = node != null ? writer.getValidator().validate( node.getParent().getName(), nodeName, sValue) : "";
		if (sError.length() > 0)
