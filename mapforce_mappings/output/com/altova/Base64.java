...
public class Base64 {
	...
	private static byte[]	Base64RevMap = new byte[128];
		static {
			for (int i=0; i<Base64RevMap.length; ++i)
				Base64RevMap[i] = -1;
			for (int i=0; i<64; ++i)
				Base64RevMap[Base64Map.charAt(i)] = (byte)i;
		}
	...
	public static byte[] decode( String sInput) throws IOException {
		...
		while (inLength > 0 && cInput[inLength - 1] == '=')
			inLength--;
		...
		while (inP < inLength) {
			...
			if (outP < outLength)
				bOutput[outP++] = (byte)out[1];
			if (outP < outLength)
				bOutput[outP++] = (byte)out[2];
		}
