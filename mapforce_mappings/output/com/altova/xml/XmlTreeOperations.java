...
public class XmlTreeOperations {
	...
	public static Document loadXmlBinary( byte[] xmlTree ) throws Exception {
		...
		try (java.io.ByteArrayInputStream io = new java.io.ByteArrayInputStream(xmlTree)) {
	...
	public static Document loadXml( String xmlString ) throws Exception {
		try (java.io.StringReader io = new java.io.StringReader(xmlString)) {
			return loadDocument(new org.xml.sax.InputSource(io));
		}
	}
	...
	public static void saveDocument( Document doc, java.io.OutputStream targetStream, String encoding, boolean bBigEndian, boolean bBOM, boolean prettyPrint, boolean omitXmlDecl ) throws Exception {
		...
		try (java.io.ByteArrayOutputStream result = new java.io.ByteArrayOutputStream()) {
			internalSave(new javax.xml.transform.stream.StreamResult(result), doc, encoding, omitXmlDecl);
	...
	public static void saveDocument( Document doc, String filename, String encoding, boolean bBigEndian, boolean bBOM, boolean prettyPrint, boolean omitXmlDecl ) throws Exception {
		try (java.io.FileOutputStream outStream = new java.io.FileOutputStream( filename, false )) {
	...
	public static byte[] saveXmlBinary( Document doc, String encoding, boolean bBigEndian, boolean bBOM, boolean prettyPrint ) throws Exception {
		...
		try (java.io.ByteArrayOutputStream result = new java.io.ByteArrayOutputStream()) {
			internalSave(new javax.xml.transform.stream.StreamResult(result), doc, encoding, false);
	...
	private static void saveXml( Document doc, boolean prettyPrint, boolean omitXmlDecl, java.io.Writer writer ) throws Exception {
		...
		try {
			internalSave(new javax.xml.transform.stream.StreamResult(writer), doc, null, omitXmlDecl);
			writer.flush();
		} catch (Exception e) {
			throw new com.altova.TargetInstanceUnavailableException("Cannot serialize.", e);
		}
	...
	protected static void internalSave(javax.xml.transform.Result result, Node node, String encoding, boolean omitXmlDecl) throws Exception {
		try {
	...
	public static void setAttribute(Element element, String nameNsUri, String name, String valueNsUri, String value) {
		...
		if (nameNsUri != null && nameNsUri.length() > 0) {
			namePrefix = findPrefixForNamespace(element, nameNsUri);
			if (namePrefix == null) {
				if (inlinedNamePrefix.length() > 0)
		...
		if (valueNsUri != null && valueNsUri.length() > 0) {
			valuePrefix = findPrefixForNamespace(element, valueNsUri);
			if (valuePrefix == null) {
	...
	public static String findPrefixForNamespace(Node node, String uri) {
		...
		/*/
		if (node.getNamespaceURI() == uri && node.getPrefix() != null && node.getPrefix() != "")
			return node.getPrefix(); // we prefer parent' prefix when child has same namespace (accoprding DOM spec. for lookupPrefix algorithm)
		...
		if (attrs != null) {
			...
			for (int index = 0 ; index < length ; ++index) {
				...
				if (att.getPrefix() != null && att.getNodeValue() != null)
					if (att.getPrefix().equals("xmlns") && att.getNodeValue().equals(uri))
						return att.getLocalName();
			}
		}
		if (node.getParentNode() != null)
			return findPrefixForNamespace(node.getParentNode(), uri);
